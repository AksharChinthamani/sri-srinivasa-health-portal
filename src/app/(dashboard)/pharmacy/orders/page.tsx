...
export default function PharmacyOrdersPage() {
  ...
  if (error) {
    return <div className="text-red-500 font-medium">{getTranslation(language, 'auto.error_loading_orders')}{(error as any).message}</div>;
  }

  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <h1 className="text-2xl font-bold text-slate-900 mb-6">{getTranslation(language, 'auto.pharmacy_orders')}</h1>
                    ...
                    <td className="p-4 max-w-xs truncate">
                      <div className="text-slate-800 font-medium flex items-center gap-2">
                        {order.isPrescription && (
                          <span className="bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded text-[10px] font-bold">Rx</span>
                        )}
                        {order.items?.map((item: any) => `${item.name || item.medicineName || item.medicine?.name} (x${item.quantity || 1})`).join(', ') || '—'}
                      </div>
